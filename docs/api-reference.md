...
| `POST` | `/action/finalize-companions` | `{ companions: Companion[] }` | `GameState` | Build starting deck from chosen companions (base cards loaded from backend) |
...
- **Finalize-companions** stamps unique runtime IDs (`{id}-{idx}`), snapshots `maxEnergy`/`maxLife`, computes `nextLevelExp`, loads base cards from `BaseCardRepository`, and builds the starting deck via `DeckService.buildStartingDeck()`. Base cards are never sent from the client.
